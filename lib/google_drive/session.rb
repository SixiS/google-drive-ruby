...
module GoogleDrive
    ...
    class Session
        ...
        def create_spreadsheet_using_old_doc_api(
            title = "Untitled",
        ...
        def create_spreadsheet(title = "Untitled")
          if @fetcher.respond_to?(:client) && @fetcher.client.present?
            ...
            if result.status == 200
              ...
              raise(
                GoogleDrive::Error,
                "Response code #{result.status} while creating spreadsheet:" +
                CGI.unescapeHTML(result.body))
            end
          else
            create_spreadsheet_using_old_doc_api(title)
          end
        end
